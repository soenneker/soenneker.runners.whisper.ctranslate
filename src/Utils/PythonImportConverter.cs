...
public class PythonImportConverter : IPythonImportConverter
{
    ...
    public async ValueTask ConvertRelativeImports(string directory)
    {
        ...
        foreach (string scriptPath in pythonFiles)
        {
            ...
            try
            {
                ...
                string[] lines = await File.ReadAllLinesAsync(scriptPath).NoSync();
                ...
                if (isModified)
                {
                    await File.WriteAllLinesAsync(scriptPath, lines).NoSync();
